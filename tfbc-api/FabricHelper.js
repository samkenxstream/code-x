...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	var request = {chaincodeId: 'tfbccc',
		fcn: 'requestLC',
		args: [req.body.taId, req.body.lcId, req.body.expiryDate, req.body.buyer, req.body.bank, req.body.seller, req.body.amount, req.body.goods, req.body.goodsCount, req.body.currency],
		chainId: 'tfbcchannel',
		txId: tx_id};
...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	var request = {chaincodeId: 'tfbccc',
		fcn: 'requestTrade',
		args: [req.body.taId, req.body.buyer, req.body.seller, req.body.amount, req.body.goods, req.body.goodsCount],
		chainId: 'tfbcchannel',
		txId: tx_id};
...
// send the transaction proposal to the peers
	...
	if (results && results[0] && results[0].status === 'SUCCESS') {
		...
		console.error('Failed to order the transaction. Error code: ' + response.status);
		res.send({code:"500", message: "Request for new Trade failed."});
	}

	if(results && results[1] && results[1].event_status === 'VALID') {
		console.log('Successfully committed the change to the ledger by the peer');
		res.send({code:"200", message: "New Trade requested successsfully."});
	} else {
		console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
	}
}).catch((err) => {
...
// Accept TradeAgreement by Seller
function acceptTrade(req, res) {
	...
	txId: tx_id};
		...
		if (results && results[0] && results[0].status === 'SUCCESS') {
			...
			console.error('Failed to order the transaction. Error code: ' + response.status);
			res.send({code:"500", message: "Trade Agreement accept failed."});
		}

		if(results && results[1] && results[1].event_status === 'VALID') {
			console.log('Successfully committed the change to the ledger by the peer');
			res.send({code:"200", message: "Trade Agreement accepted successsfully."});
		} else {
			console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
			res.send({code:"500", message: "Trade Agreement accept failed."});
		}
...
function sendShipment(req, res) {
	...
	txId: tx_id};
		...
		if (results && results[0] && results[0].status === 'SUCCESS') {
			...
			console.error('Failed to order the transaction. Error code: ' + response.status);
			res.send({code:"500", message: "Sending Shipment failed."});
		}

		if(results && results[1] && results[1].event_status === 'VALID') {
			console.log('Successfully committed the change to the ledger by the peer');
			res.send({code:"200", message: "Shipment Sent successsfully."});
		} else {
			console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
			res.send({code:"500", message: "Sending Shipment failed."});
		}
...
function receiveShipment(req, res) {
	...
	if (results && results[0] && results[0].status === 'SUCCESS') {
		...
		console.error('Failed to order the transaction. Error code: ' + response.status);
		res.send({code:"500", message: "Status change for Receive Shipment failed."});
	}

	if(results && results[1] && results[1].event_status === 'VALID') {
		console.log('Successfully committed the change to the ledger by the peer');
		res.send({code:"200", message: "Shipment received and status changed successsfully."});
	} else {
		console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
	}
}).catch((err) => {
...
function requestPayment(req, res) {
	...
	txId: tx_id};
		...
		if (results && results[0] && results[0].status === 'SUCCESS') {
			...
			console.error('Failed to order the transaction. Error code: ' + response.status);
			res.send({code:"500", message: "Payment Request failed."});
		}

		if(results && results[1] && results[1].event_status === 'VALID') {
			console.log('Successfully committed the change to the ledger by the peer');
			res.send({code:"200", message: "Payment Request Sent successsfully."});
		} else {
			console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
			res.send({code:"500", message: "Payment Request failed."});
		}
...
function makePayment(req, res) {
	...
	txId: tx_id};
		...
		if (results && results[0] && results[0].status === 'SUCCESS') {
			...
			console.error('Failed to order the transaction. Error code: ' + response.status);
			res.send({code:"500", message: "Payment to Seller failed."});
		}

		if(results && results[1] && results[1].event_status === 'VALID') {
			console.log('Successfully committed the change to the ledger by the peer');
			res.send({code:"200", message: "Payment to Seller sent successsfully."});
		} else {
			console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
		}
	}).catch((err) => {
...
function getLCHistory(req, res){
	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		if (query_responses && query_responses.length == 1) {
			if (query_responses[0] instanceof Error) {
				console.error("error from query = ", query_responses[0]);
				res.send({code:"500", message: "Issue with getting LC History details"});
			} else {
...
// Get current state of TA using Buyer user
function getTA(req, res){
	//Init fabric client
	var fabric_client = new Fabric_Client();
	...
	if (user_from_store && user_from_store.isEnrolled()) {
		...
		if (query_responses && query_responses.length == 1) {
			if (query_responses[0] instanceof Error) {
				console.error("error from query = ", query_responses[0]);
				res.send({code:"500", data: "Issue with getting TA details"});
			} else {
			...
			console.log("No payloads were returned from query");
			res.send({code:"500", data: "No TA found"});
		}
	}).catch((err) => {
		console.error('Failed to query successfully :: ' + err);
		res.send({code:"500", data: "Issue with getting TA details"});
...
function getTAHistory(req, res){
	...
	if (user_from_store && user_from_store.isEnrolled()) {
		...
		if (query_responses && query_responses.length == 1) {
			if (query_responses[0] instanceof Error) {
				console.error("error from query = ", query_responses[0]);
				res.send({code:"500", message: "Issue with getting TA History details"});
			} else {
			...
			console.log("No payloads were returned from query");
			res.send({code:"500", message: "No TA found"});
		}
	}).catch((err) => {
		console.error('Failed to query successfully :: ' + err);
		res.send({code:"500", message: "Issue with getting TA History details"});
